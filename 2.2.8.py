...
число m — количество новых сотрудников, которым нужно раздать корпоративные ящики. Каждая из последующих m строк
...
emails = [input().split('@')[0] for i in range(int(input()))]
new_emails = [input().split('@')[0] for i in range(int(input()))]

for mail in new_emails:
    if f'{mail}' not in emails:
        emails.append(mail)
        print(f'{mail}@beegeek.bzz')
    else:
        for i in range(1, 100):
            if f'{mail}{i}' not in emails:
                emails.append(f'{mail}{i}')
