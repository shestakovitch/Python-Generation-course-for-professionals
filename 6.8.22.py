...
n — количество покупателей. В последующих n строках вводятся два числа, разделенные пробелом, где первое число является
...
textbooks, buyers, s = Counter(map(int, input().split())), [], 0
for i in range(int(input())):
    buyers.append(list(map(int, input().split())))
for book, price in buyers:
    if book in +textbooks:
        s += price
        textbooks.subtract({book: 1})
print(s)