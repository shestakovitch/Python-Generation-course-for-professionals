...
with open('pools.json', encoding='utf-8') as in_data:
    data = json.load(in_data)
    res = []
    for item in data:
        if '10:00-12:00' <= item['WorkingHoursSummer']['Понедельник'] <= '10:00-24:00':
            res.append((item['DimensionsSummer']['Length'], item['DimensionsSummer']['Width'], item['Address']))
    l, w, address = sorted(res, key=lambda x: x[0])[-1]
    print(f'{l}x{w}\n{address}')