...
заведений в нем определяет сеть с самым большим числом заведений и выводит название этой сети и количество заведений
этой сети
...
with open('food_services.json', encoding='utf-8') as in_data:
    data = json.load(in_data)
    dist_data, name_data = {}, {}
    for item in data:
        dist_data[item['District']] = dist_data.get(item['District'], 0) + 1
    for item in data:
        if item['OperatingCompany'] != '':
            name_data[item['OperatingCompany']] = name_data.get(item['OperatingCompany'], 0) + 1
    print(*sorted(dist_data.items(), key=lambda x: -x[1])[0], sep=': ')
    print(*sorted(name_data.items(), key=lambda x: -x[1])[0], sep=': ')