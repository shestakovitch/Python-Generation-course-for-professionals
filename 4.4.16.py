...
with open('food_services.json', encoding='utf-8') as in_data:
    ...
    bigtype_data = {}
    for item in data:
        bigtype_data.setdefault(item['TypeObject'], []).append((item['Name'], item['SeatsCount']))
    for k, v in sorted(bigtype_data.items(), key=lambda x: x[0]):
        print(f'{k}: ', end='')
        print(*max(v, key=lambda x: x[1]), sep=', ')